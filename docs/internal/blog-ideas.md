...
### 次の候補 — タブアイコンを作り直す（2026-09-24、連載番号未定）

- **記事の主題**: サイトのタブにアイコンが見えないことに気づき、表示経路と小サイズでの可読性を検証しながら独自faviconを整える。**原因は未特定**であり、調査前に「フォントのせい」と断定しない。
- **実際に確認したこと**: サイトには `/favicon.svg` の指定とSVGファイルが既にある。既存SVGは外部フォントで「A」を描いており、`favicon.ico` という名前のファイルの実体はPNGだった。
- **Computerの準備**: フォント不要のパス描画「A」（黄地・黒文字）のSVG、複数サイズを含む正しいICO、小サイズの見た目確認PNGを作成。`docs/internal/design-mocks/devrev-aruaru-favicon-handoff.md` に検証・Cursor受け渡しの手順を記録。**まだ本番適用していない**。
- **書ける切り口**: 「アイコンを置いた」だけでは終わらず、faviconの形式、MIME、キャッシュ、16pxでの可読性まで確認する。Computerがデザインを用意し、Cursorが実装・ブラウザ検証する分業。
- **匿名化**: 実組織、リポジトリ名、ISS番号、デプロイ先は公開原稿から除く。
- **ステータス**: idea（実ブラウザ確認待ち）
- **DevRev Issue**: (未起票。サイト改修として着手するならComputerがPROD-11配下に起票)
